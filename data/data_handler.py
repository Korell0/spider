...
@database_common.connection_handler
def get_cart_contet(cursor, user_id):
    cursor.execute("""SELECT spiders.spider_name FROM users JOIN orders ON users.id = orders.user_id JOIN 
    spiders ON spiders.id = orders.spider_id WHERE orders.user_id = %(user_id)s;""", {'user_id': user_id})
